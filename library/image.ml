...
(* relativeToAbsolute : (float * float) list -> float * float -> (float * float) list *)
...
(* get_min : (float * float) list -> (float * float) -> (float * float)
...
(* get_max : (float * float) list -> (float * float) -> (float * float)
   NB: points are expected to be absolute points not relative *)
...
(* text_wh : string -> float -> (float * float) *)
...
(* empty : float -> int -> Image.t *)
let empty width height =
  rectangle width height Color.white ~fill:true ~outline_size:0.

(* circle : float -> ?fill:bool -> ?outline_size:int -> Color.t -> Image.t *)
...
(* moveImage : float -> int -> Image.t -> Image.t *)
...
(* placeImages : Image.t list -> (float * float) list -> Image.t -> Image.t *)
let rec placeImages images positions background =
  match (images, positions) with
  | ([], []) -> background
  | ([], _) -> failwith "placeImages: too many posns."
  | (_ , []) -> failwith "placeImages: too many pics."
...
(* placeImage : Image.t -> float * float -> Image.t -> Image.t *)
...
(* drawOutline : Cairo.context -> float -> unit *)
...
(* drawLine : (float * float) list -> Cairo.context -> unit *)
...
let rec draw context image =
  ...
  | Rect {color; x1; y1; x2; y2; fill; outline_size} ->
    ...
    Cairo.rectangle context x1 y1 ~w ~h;
...
let make_corners (min_posn, max_posn) outline_size =
  let half_size = outline_size /. 2.0
  in
...
let rec corner_lst images =
  ...
  | Picture {picture; x; y} :: rest ->
    let x' = x +. (float (Cairo.Image.get_width picture)) in
    let y' = y +. (float (Cairo.Image.get_height picture))
    in
